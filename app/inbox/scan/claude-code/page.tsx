...
import { scanClaudeCodeDirectoryAction } from "@/app/inbox/scan/claude-code/actions";
import { ClaudeCodeJsonlScanPreview } from "@/components/inbox/claude-code-jsonl-scan-preview";
...
export default function ClaudeCodeScanPage() {
  ...
  async function handleScan() {
    ...
    try {
      const response = await scanClaudeCodeDirectoryAction(directoryPath.trim());
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold">扫描 Claude Code 对话目录</h1>
          <p className="text-sm text-muted-foreground">
            递归扫描本地 Claude Code `.jsonl` 对话文件，并标出哪些文件是新文件、哪些已经变更。
          </p>
        </div>
      ...
      {result ? <ClaudeCodeJsonlScanPreview {...result} /> : null}
    </div>
  );
}
