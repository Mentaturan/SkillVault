...
export const claudeCodeJsonlImportPathSchema = z.object({
  sourcePath: z
    .string()
    .min(1, "文件路径不能为空")
    .max(2000, "文件路径过长")
...
export type ClaudeCodeJsonlImportPathInput = z.infer<typeof claudeCodeJsonlImportPathSchema>;

export const claudeCodeJsonlScanPathSchema = z.object({
  directoryPath: z.string().min(1, "目录路径不能为空").max(2000, "目录路径过长"),
});

export type ClaudeCodeJsonlScanPathInput = z.infer<typeof claudeCodeJsonlScanPathSchema>;
