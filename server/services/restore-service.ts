  ...
  const result = db.transaction((tx) => {
    ...
    for (const assetPreview of preview.assets) {
      ...
      const assetRecord: Omit<NewAsset, "id"> = {
        ...
        sourceRef: bundleAsset.metadata.sourceRef ?? null,
        sourcePath: bundleAsset.metadata.sourcePath ?? null,
        sourceImportedAt: bundleAsset.metadata.sourceImportedAt ?? null,
        sourceChecksum: bundleAsset.metadata.sourceChecksum ?? null,
