...
async function collectJsonlFiles(directoryPath: string, files: string[] = []) {
...
export async function scanClaudeCodeJsonlDirectory(directoryPath: string) {
  ...
  const files = await collectJsonlFiles(directoryPath);
  if (files.length > MAX_SCAN_FILES) {
    throw new Error(`扫描结果超过 ${MAX_SCAN_FILES} 个文件，请缩小目录范围。`);
  }

  const snapshots = await getCaptureImportFileSnapshots("claude_code_jsonl", files);
