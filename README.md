...
代码功能已覆盖本地路线图 v0.1-alpha 到 v0.9 的全部范围。v0.9 新增了 GitHub 仓库归档导入（支持路径过滤和文件数量限制）、来源更新检查（比较远端 checksum 不自动覆盖）、本地精选示例、资产列表来源筛选和资产详情来源信息独立 Card。`package.json` 仍显示 `0.1.0-alpha`，这代表发布版本号尚未重切，不代表代码功能只停留在 alpha。
