...
代码功能现在已经进入本地路线图的 v0.9 早期阶段：v0.8 已完成；v0.9 的第一刀已经补上 imported asset 的来源元数据，包括 URL、ref、path、imported-at 和 checksum，并贯通到 Markdown 导入、详情展示、备份恢复和 Markdown 导出。下一刀是 raw GitHub file import 的预览与校验。`package.json` 仍显示 `0.1.0-alpha`，这代表发布版本号尚未重切，不代表代码功能只停留在 alpha。
