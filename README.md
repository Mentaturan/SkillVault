...
代码功能现在已经推进到本地路线图的 v0.8 完成状态：v0.7 的 capture inbox 和本地对话挖掘已完成；v0.8 的基础版本 diff、基于复制行为的 `lastUsedAt` 更新、手动 `reviewDueAt`/复查队列，以及资产专项筛选已完成。额外的 model/tool compatibility 字段没有进入本阶段，因为现有 `type` 和 `targetTool` 已能支撑筛选，而再加字段会先让表单变重。下一阶段转入 v0.9 的 source metadata 和手动导入来源。`package.json` 仍显示 `0.1.0-alpha`，这代表发布版本号尚未重切，不代表代码功能只停留在 alpha。
