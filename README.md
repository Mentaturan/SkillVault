...
代码功能现在已经推进到本地路线图的 v0.8 后半段：v0.7 的 capture inbox 和本地对话挖掘已完成；v0.8 的基础版本 diff、基于复制行为的 `lastUsedAt` 更新、手动 `reviewDueAt`/复查队列，以及资产专项筛选已完成。当前剩余的主要任务是判断 model/tool compatibility 字段是否值得进入表单。`package.json` 仍显示 `0.1.0-alpha`，这代表发布版本号尚未重切，不代表代码功能只停留在 alpha。
