...
- 资产详情和导入预览中的确定性校验结果展示，覆盖 SKILL.md 元数据和变量占位符语法
- 移动端浏览器的基础查看、搜索和复制体验

代码功能现在已经推进到本地路线图的 v0.4：备份、恢复和数据完整性已完成。`package.json` 仍显示 `0.1.0-alpha`，这代表发布版本号尚未重切，不代表代码功能只停留在 alpha。
...
确定性校验和安全检查：第一轮已完成校验结果模型、SKILL.md 必填元数据、正文长度、重复名、unsupported metadata 和变量占位符语法检查，并显示在资产详情和导入预览中。
