...
代码功能现在已经进入本地路线图的 v0.9 中段：v0.8 已完成；v0.9 已补上 imported asset 的来源元数据，以及公开 GitHub Markdown 文件的 URL 预览导入流程，包含预览、校验、导入前 checksum 复核和来源信息写回。下一刀是带路径过滤和文件数限制的 repository archive import。`package.json` 仍显示 `0.1.0-alpha`，这代表发布版本号尚未重切，不代表代码功能只停留在 alpha。
