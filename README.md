...
- Capture Inbox：手动粘贴、review 后转资产，以及本地 Codex rollout `.jsonl` 预览导入
- 移动端浏览器的基础查看、搜索和复制体验

代码功能现在已经推进到本地路线图的 v0.7：capture inbox 和本地对话挖掘进行中，已完成手动采集、review 转资产和 Codex rollout 导入，下一步是增量扫描和 changed-file detection。`package.json` 仍显示 `0.1.0-alpha`，这代表发布版本号尚未重切，不代表代码功能只停留在 alpha。
