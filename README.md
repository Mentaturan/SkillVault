...
代码功能现在已经推进到本地路线图的 v0.8 早期阶段：v0.7 的 capture inbox 和本地对话挖掘已完成，v0.8 的基础版本 diff 和基于复制行为的 `lastUsedAt` 更新已完成，接下来是 test runs 和更完整的 use history/filtering。`package.json` 仍显示 `0.1.0-alpha`，这代表发布版本号尚未重切，不代表代码功能只停留在 alpha。
